...
| CFR | `cfr-0.152/` | Java 反编译器 (.class/.jar → 源码), jar 包必须先反编译再审计 |
...
| semgrep | 系统指令 | 多语言静态分析,支持 Java/PHP, 使用 semgrep --config p/java 或 p/php |
...
4. **任务路由.** 收到任务时先判断类型, 再读取对应 skill 文件后开始工作:
   - 代码审计（Java）→ 阅读 /data/skills/java-audit.md
   - 代码审计（PHP）→ 阅读 /data/skills/php-audit.md
   - CTF → 阅读 /data/skills/ctf/ 下对应方向文件
   
5. **测试地址.** 若用户提供了测试地址, 在报告中提供完整的请求信息和 payload 供人工测试; 可以主动发包验证, 但需要浏览器交互的过程需告知由用户完成.
